...
bool isValidBST(Node* node)
{
    ...
    //if max value in left subtree is bigger than root then false
    if(node->left != NULL && findMaximum(node->left)->ISBN >val) return false;
    //if max value in left subtree is bigger than root then false
    if(node->right != NULL && findMinimum(node->right)->ISBN <val) return false;

    //now recurcively check if both left & right subtrees are BST or not
