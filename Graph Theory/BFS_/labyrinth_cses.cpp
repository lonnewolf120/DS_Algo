...
void bfs(int si, int sj)
{
	...
	while(!q.empty())
	{
		...
		for(auto i:vp)
		{
			...
			if(mx<0 || my<0 || mx>n || my>m || grid[mx][my]==0 || visited[mx][my]) continue;
			visited[mx][my]=true;
			q.push({mx,my});
			grid[mx][my]=0;
...
void solve(){
	...
	forn(i,n)
	{
		if(i==0 || i==n-1)
		{
			forn(j,m)
			{
				if(grid[i][j]=='.' && visited[i][j]==true){f=true;}
			}
		}
		if(grid[i][m-1]=='.' && visited[i][m-1]==true)f=true;
		if(grid[i][0]=='.' && visited[i][0]==true)f=true;
	}
	if(f)
	{
		yes;
