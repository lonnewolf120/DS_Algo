...
void dfs(ll v, ll parent, vector<vector<ll>> &tree, vector<ll> &subtree_sizes)
{
    subtree_sizes[v] = 1;
    for (ll u : tree[v])
    {
...
int32_t main()
{
    ...
    dfs(1, 0, tree, subtree_sizes);

    vector<pair<ll, ll>> edges;
    for (ll i = 1; i <= n; i++)
    {
        for (ll u : tree[i])
            (subtree_sizes[u] < subtree_sizes[i])? edges.push_back({i, u}) : edges.push_back({u, i});
    }
    sort(all(edges));
    for (ll i = 0; i < n - 1; i++)
