...
int32_t main()
{
    ...
    ll n,k,x;
    cin>>n>>k>>x;
    if(n<k){cout<<-1<<ln;continue;}
    if(x<(k-1)){cout<<-1<<ln;continue;}
    vector<ll> v;
    for(ll i=0;i<k;++i) v.push_back(i);
    if(x>k) for(ll i=x;v.size()<n;) v.push_back(i);
    else for(ll i=k-1;v.size()<n;) v.push_back(i);
    ll s = 0;
    for(auto it:v) s+=it;
    cout<<s<<endl;
    }
