...
// --------------------<DEBUG>--------------------------
template <typename T>
ostream &operator<<(ostream &os, vector<T> &llermediate_array)
{
    for (auto &a : llermediate_array)
        cout << a << ' ';
    return os;
}
template <typename T>
istream &operator>>(istream &is, vector<T> &llermediate_array)
{
    for (auto &a : llermediate_array)
        cin >> a;
    return is;
...
{
    ll cnt = 0, sum = 1;
    for (ll i = 0; prime[i] * prime[i] <= n and i < nprime; i++)
    {
        cnt = 0;
        while (n % prime[i] == 0)
        {
...
int32_t main()
{
    ...
    tst
    {
        ...
        cin >> n >> k;
        vector<ll> a(n);
        vector<ll> initial(k + 1, -1); //first occurrence of each color.
        vector<ll> last(k + 1, -1);  //last occurrence of each color.
        vector<ll> mxWidth(k + 1, -1), mxHeight(k + 1, -1);

        for (ll i = 0; i < n; i++)
        {
            cin >> a[i];
            if (initial[a[i]] == -1) initial[a[i]] = i;
            last[a[i]] = i;
        }
        for (ll i = 0; i < n; i++)
        {
            ll color = a[i];
            mxWidth[color] = max(mxWidth[color], i - initial[color] + 1);
            mxHeight[color] = max(mxHeight[color], last[color] - i + 1);
        }

        for (ll color = 1; color <= k; color++)
        {
            ll ans = mxWidth[color] + mxHeight[color] - 1;
            cout << ans << " ";
        }
        cout << ln;
    }
